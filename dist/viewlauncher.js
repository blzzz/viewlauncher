...
(function() {
  define('cs!viewcollection',['jquery', 'underscore'], function($, _) {
    var ViewCollection;
    return ViewCollection = (function() {
      ...
      ViewCollection.prototype.waitFor = function(fncName, context, next, options, maxDuration) {
        var actions, clock, useArray;
        if (context == null) {
          context = this;
        }
        if (maxDuration == null) {
...
(function() {
  ...
  define('cs!viewloader',['require', 'underscore', 'backbone', 'jquery', 'cs!viewcollection'], function(require, _, Backbone, $, ViewCollection) {
    var ViewLoader;
    return ViewLoader = (function(_super) {
      ...
      ViewLoader.prototype.detectAndLoad = function(views, $html, context, next) {
        var detectedViews, loader, requirePaths, view;
        detectedViews = (function() {
          var _i, _len, _results;
          _results = [];
          for (_i = 0, _len = views.length; _i < _len; _i++) {
            view = views[_i];
            if ($html.is(":has(" + view.selector + ")")) {
              _results.push(view);
            } else {
              continue;
            }
        ...
        if (requirePaths.length === 0) {
          ...
          require(requirePaths, function() {
            var Prototype, i, _i, _len;
            for (i = _i = 0, _len = detectedViews.length; _i < _len; i = ++_i) {
              view = detectedViews[i];
              Prototype = arguments[i];
              $html.find(view.selector).each(function(i) {
                var config, defaultCycle, defaultModule, extension;
                config = _.extend({
                  el: $(this)
                }, view);
                ...
                return loader.push(new Prototype(config));
              });
            }
            return loader.loadInstances(context, next);
          });
        }
        return this;
      };

      ViewLoader.prototype.loadInstances = function(context, next) {
        return this.waitFor(['cycle', 'load'], context, next);
      };
...
// Generated by CoffeeScript 1.7.1
(function() {
  define('cs!section',['backbone', 'cs!viewcollection', 'cs!viewloader', 'exports', 'require', 'imagesLoaded'], function(Backbone, ViewCollection, ViewLoader, exports, require) {
    return exports.Section = Backbone.View.extend({
      sections: null,
      views: null,
      contents: null,
      ContentPrototype: Backbone.View.extend({
        ...
        to: function(stateName, duration) {
          ...
          state = (_ref = this.config.states) != null ? _ref[stateName] : void 0;
          if (typeof state === 'string') {
            return this.$el.removeClass(this.config.states.join(' ')).addClass(state);
          } else {
            if (duration === 0) {
              this.$el.css(state);
              return this;
      ...
      events: {
        'click a[href]': function(e) {
          return (this.config.launcher || this).requestClickedLink(e, this);
        }
      },
      initialize: function(config) {
        this.config = config;
      ...
      findAndLoad: function(next, isTriggerSection) {
        var $el, config, isLauncher, launchable, launchables, launcher, section, sectionSelector, sectionsLaunchables, selector, _ref;
        isLauncher = !this.config.launcher;
        launcher = isLauncher ? this : this.config.launcher;
        sectionsLaunchables = {
          sections: [],
          views: []
        };
        ...
        launcher.trigger('launchablesRequested', this.el, sectionsLaunchables);
        if (!this.contents) {
          this.render();
        }
        $el = this.contents.last().$el;
        return this.loadSections($el, sectionsLaunchables.sections, function() {
          launcher.trigger('sectionsLoaded', this.sections.get('el'), this.el);
          return this.sections.waitFor('findAndLoad', this, function() {
            return this.loadViews($el, sectionsLaunchables.views, function() {
              launcher.trigger('viewsLoaded', this.views.views, this.el);
              if (isTriggerSection === true) {
                return this.sections.waitFor('playTransition', this, function() {
                  this.views.launchInstances(false);
                  this.trigger('launchSectionReady', this.views);
                  return next.call(this);
                });
              } else {
      ...
      render: function(content) {
        var isInitial, isLauncher, sectionContent;
        if (content == null) {
          content = this.$el.html();
        }
        isLauncher = !this.config.launcher;
        isInitial = !this.contents;
        if (isInitial && !isLauncher) {
          this.$el.empty();
        }
        sectionContent = new this.ContentPrototype({
          states: this.transitionStates,
          className: (this.config.launcher || this).config.sectionContentClassName,
          html: content
        });
      ...
      loadSections: function($el, sections, next) {
        var detectedSections, launcher, parentSection, requiredSections, section, sectionsLoaded;
        detectedSections = (function() {
          var _i, _len, _results;
          _results = [];
          for (_i = 0, _len = sections.length; _i < _len; _i++) {
            section = sections[_i];
            if ($el.is(":has(" + section.selector + ")")) {
        ...
        sectionsLoaded = function() {
          var extension, i, stateExtension, _i, _len;
          if (arguments) {
          ...
          parentSection.sections = parentSection.contents.last().sections = new ViewCollection().reset((function() {
            var _j, _len1, _results;
            _results = [];
            for (_j = 0, _len1 = detectedSections.length; _j < _len1; _j++) {
              section = detectedSections[_j];
          ...
          return next.call(parentSection);
        };
        if (requiredSections.length === 0) {
          return sectionsLoaded.call(this);
        } else {
          return require(_.pluck(requiredSections, 'source'), sectionsLoaded);
        }
      },
      loadViews: function($el, views, next) {
        return this.views = this.contents.last().views = new ViewLoader().detectAndLoad(views, $el, this, function() {
          var section;
          section = this;
      ...
      loadContentAssets: function(next) {
        if (this.contents) {
          return this.contents.last().$(this.config.imagesToLoad).imagesLoaded(next);
        } else {
          return next.call(this);
        }
      },
      reloadAll: function(page, next) {
        return this.reloadSections(page, function() {
          this.views.updateInstances(page, this.$el);
          return this.sections.waitFor('playTransition', this, next);
        });
      },
      reloadSections: function(page, next) {
        ...
        this.sections.each(function(section) {
          return section.render(page.sync(section.config.selector, $el).html2());
        });
...
// Generated by CoffeeScript 1.7.1
(function() {
  define('cs!pagecollection',['backbone', 'underscore', 'jquery'], function(Backbone, _, $) {
    return Backbone.Collection.extend({
      config: {},
      SelectorSync: (function() {
        function _Class($el1, $el2, path) {
          this.$el1 = $el1;
          this.$el2 = $el2;
          this.path = path;
          this;
        ...
        _Class.prototype.html1 = function() {
          return this.$el1.html();
        };

        _Class.prototype.html2 = function() {
          return this.$el2.html();
        };

        _Class.prototype.find = function(path) {
          return this.find1(path).add(this.find2(path));
        };

        _Class.prototype.find1 = function(path) {
          if (path == null) {
            path = '';
          }
        ...
        _Class.prototype.find2 = function(path) {
          if (path == null) {
            path = '';
          }
        ...
        _Class.prototype.sameSize = function(path, filter) {
          if (filter == null) {
            filter = '*';
          }
        ...
        _Class.prototype.readAttributes = function($target, allowedAttrs) {
          var attrs;
          attrs = [];
          $target.each(function() {
            var a, props, _i, _len, _ref;
            props = {};
            _ref = this.attributes;
            for (_i = 0, _len = _ref.length; _i < _len; _i++) {
              a = _ref[_i];
              if (!allowedAttrs || _.indexOf(allowedAttrs, a.name) >= 0) {
        ...
        _Class.prototype.manipulateContent = function($target, $source, type) {
          if (type == null) {
            type = 'html';
          }
          return $target.each(function(i) {
            return $(this)[type]($source.eq(i)[type]());
          });
        };

        _Class.prototype.manipulateAttributes = function($target, attrs) {
          return $target.each(function(i) {
            return $(this).removeAttr(_.keys(attrs[i]).join(' ')).attr(attrs[i]);
          });
        };

        _Class.prototype.$ = function(path, filter, replaceContent, replaceAttributes) {
          var $el1, $el2, sync;
          if (path == null) {
            path = '';
          }
          ...
          return this.find(path).filter(filter).each(function() {
            var $source, $target, index;
            index = $(this).index();
            $target = $el1.eq(index);
            $source = $el2.eq(index);
            if (replaceContent) {
              sync.manipulateContent($target, $source);
            }
            if (replaceAttributes) {
              return sync.manipulateAttributes($target, sync.readAttributes($source));
            }
          });
        };

        _Class.prototype.contentOf = function(path, type) {
          if (path == null) {
            path = '';
          }
          if (!this.sameSize(path)) {
          ...
          this.manipulateContent(this.find1(path), this.find2(path), type);
          return true;
        };

        _Class.prototype.htmlOf = function(path) {
          return this.contentOf(path);
        };

        _Class.prototype.textOf = function(path) {
          return this.contentOf(path, 'text');
        };

        _Class.prototype.attributesOf = function(path, allowedAttrs) {
          if (path == null) {
            path = '';
          }
          if (!this.sameSize(path)) {
          ...
          this.manipulateAttributes(this.find1(path), this.readAttributes(this.find2(path), allowedAttrs));
          return true;
        };

        _Class.prototype.classesOf = function(path) {
          return this.attributesOf(path, ['class']);
        };

        _Class.prototype.idsOf = function(path) {
          return this.attributesOf(path, ['id']);
        };

        _Class.prototype.stylesOf = function(path) {
          return this.attributesOf(path, ['style']);
        };
      ...
      model: Backbone.Model.extend({
        ...
        sync: function(path, $el) {
          ...
          return new this.collection.SelectorSync($el.find(path), this.$(path), path);
        }
      }),
...
// Generated by CoffeeScript 1.7.1
(function() {
  define('cs!launcher',['cs!./section', 'cs!./pagecollection'], function(LaunchableSection, PageCollection) {
    return LaunchableSection.extend({
      currPage: null,
      nextPage: null,
      pages: null,
      router: null,
      ...
      start: function() {
        ...
        Router = Backbone.Router.extend({
          ...
          request: function(href) {
            return launcher.requestPage(href, true);
          }
        });
      ...
      requestClickedLink: function(e, section) {
        ...
        if (href.prop.slice(0, root.length) === root) {
          ...
          return this.requestPage(href.attr, false, section);
        }
      },
      requestPage: function(href, byRoute, clickSection) {
        var byCall, byClick, _ref;
        if (href == null) {
          href = '';
        }
        if (byRoute == null) {
        ...
        byClick = !byRoute && clickSection !== null;
        byCall = !byRoute && !byClick;
        if (this.loading && this.loading.state() === 'pending') {
          return;
        }
        if (((_ref = this.currPage) != null ? _ref.get('href') : void 0) === href) {
          return;
        }
        if (byClick) {
        ...
        return this.pages.byHref(href, this, function(page) {
          var done, launcher, loading, section, _base;
          ...
          launcher.nextPage = page;
            ...
            launcher.trigger('transitionDone', this.el);
            return loading.resolve(launcher);
          };
          switch (false) {
            case !byCall:
              return this.findAndLoad(done, true);
            case !byClick:
              section = clickSection.sections.length > 0 ? clickSection : this;
              return section.reloadAll(page, done);
            case !byRoute:
              section = typeof (_base = this.config).getSectionByPageRoute === "function" ? _base.getSectionByPageRoute(launcher.currPage, page, this) : void 0;
              return (section || this).reloadAll(page, done);
          }
        });
      }
    });
